...
Branch: `workstream-a-marketing` · Current HEAD: `e213925` · Rollback for whole workstream: `b86e06e`
Origin: `e213925` — pushed 2026-08-10, fast-forward `f82430c..e213925`. **Branch and origin are level; nothing unpushed.**

⛔ **`main` IS AT `358d615`. IT MOVED 2026-08-10 — the first time since July 30, and the "unchanged since July 30" statement this section carried is now FALSE.** `main` advanced `be2dd14..358d615` via **usrad-platform PR #4** (`ef3ba9a`, `361b646`) under the **authorized EDS containment exception**. Vercel production is READY on `358d615` and usrad.com serves it.

⛔ **NO FAST-FORWARD IS AVAILABLE.** The branch forked at `be2dd14` and is now **61 commits ahead and 3 behind**. **Three files conflict**, each edited by both sides from the common base: `ROICalculator.astro`, `api/employer-guide-download.ts`, `api/employer-roi-report.ts` — verified by simulated merge, not assumed. A **RECONCILIATION BATCH** runs before F, G and H under the binding rules in `DECISIONS.md` 2026-08-10, *"RECONCILIATION RULES — BINDING ON THE MERGE BATCH."*
...
| **RECONCILIATION** | ⛔ **NEW — inserted 2026-08-10, DOC-19. RUNS AFTER DOC-19 COMMITS AND BEFORE F, G, H.** Merge `origin/main` (`358d615`) into the branch and resolve the three conflicting files under the seven binding reconciliation rules. Stops after resolution, before commit |
| **E** | Calculator (D6, D13) — **MERGE GATE. ⛔ RESCOPED 2026-08-10, DOC-19.** Containment reduced its scope; **it did NOT discharge D13.** The download promise and the PDF route are gone upstream. **E still owns:** the live on-page savings presentation · the `$350`-derived calculator arithmetic · the related savings and bar geometry and the production-only waterfall and split logic · the remaining `projectedSavings` payload and data-contract plumbing · the final calculator CTA and presentation replacing the temporary containment copy |
| **F**, **G**, **H** | Guide landing page (D12) · `/employer/schedule` · `EmployerConsultationForm` |
| **AnciCareLegacy** | ⛔ **OMITTED FROM THE SEQUENCE ABOVE — added 2026-08-10, DOC-18.** ✅ **UNBLOCKED 2026-08-10, DOC-19** — #50 resolved. **This is the NEXT SURFACE WORK, ahead of F.** Its own short corrective batch. See below and open item #49 |
