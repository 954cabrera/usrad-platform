/**
 * ⚠️  CRITICAL: TRIPLE-FILE SYSTEM - READ BEFORE EDITING ⚠️
 * ==========================================================
 * 
 * THIS FILE EXISTS IN THREE LOCATIONS:
 * 
 * 1. src/lib/procedures/data/procedure-data.js  ← SOURCE OF TRUTH (edit here!)
 * 2. public/js/procedure-data.js                ← Astro browser runtime copy
 * 3. [REMIX]/app/lib/procedure-data.js          ← Remix server-side copy
 * 
 * 🚨 ONLY EDIT THE SOURCE OF TRUTH! 🚨
 * ------------------------------------
 * All changes should be made in location #1, then copied to #2 and #3.
 * 
 * WHY THREE COPIES EXIST:
 * -----------------------
 * • #1 (src/lib): Astro build-time TypeScript imports, universal search engine
 * • #2 (public/js): Browser runtime for window.ProcedureLibrary, HeroSection
 * • #3 (Remix): Server-side rendering, CSR Console, PBS booking system
 * 
 * ✅ CORRECT WAY TO UPDATE:
 * -------------------------
 * 1. Edit: src/lib/procedures/data/procedure-data.js  ← Edit the source!
 * 
 * 2. Copy to Astro public folder:
 *    cp src/lib/procedures/data/procedure-data.js public/js/procedure-data.js
 * 
 * 3. Copy to Remix app:
 *    cp src/lib/procedures/data/procedure-data.js ../usrad-portal/app/lib/procedure-data.js
 * 
 * 4. Test locally:
 *    - Astro: npm run dev → Check browser console for "✓ Procedure Library loaded"
 *    - Remix: npm run dev → Test CSR Console procedure search
 * 
 ...
 *    
 *    # In Remix repo:
 *    git add app/lib/procedure-data.js
 *    git commit -m "feat: Sync procedure data from Astro - [describe changes]"
 * 
 * ⚠️  IF YOU EDIT A COPY DIRECTLY:
 * --------------------------------
 * • Changes will be LOST on next sync
 * • Apps will have inconsistent procedure data
 * • Search results won't match across platforms
 * • Creates debugging nightmares
 * 
 * 🔗 FILES THAT USE THIS:
 * -----------------------
 * ASTRO (public/js):
 * • HeroSection.astro - Browser import for window.ProcedureLibrary
 * • Inline search script - Accesses window.ProcedureLibrary.Popular
 * 
 * ASTRO (src/lib):
 * • universal-search-index.ts - TypeScript import for search algorithm
 * • universal-search-engine.ts - Indirectly via search index
 * 
 * REMIX (app/lib):
 * • cpt-index.server.ts - Builds CPT lookup index
 * • procedure-search.server.ts - Comprehensive procedure search
 * • procedure-resolver.ts - Resolves procedures by CPT + body part
 * • pbs.search.tsx - Patient booking search
 * • csr.new-booking.tsx - CSR Console booking
 * 
 * 📚 FOR MORE INFO:
 * -----------------
 * See: USRad_Procedure_Library_System_Architecture_COMPLETE.md
 * Section: "The Dual-Import System Explained" (now Triple-Import)
 * 
 * COMPREHENSIVE IMAGING PROCEDURE LIBRARY
 * =======================================
 * Complete CPT reference - 195 procedures across MRI, CT, X-Ray, Ultrasound
 * 
 * Last Updated: December 31, 2025
 * Source File: src/lib/procedures/data/procedure-data.js
 * Architecture: ES6 module with window object for backward compatibility
 */
