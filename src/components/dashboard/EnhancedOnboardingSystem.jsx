...
const EnhancedOnboardingSystem = () => {
  ...
  // NEW: Handle PSA completion from redirect
  useEffect(() => {
    const urlParams = new URLSearchParams(window.location.search);
    const psaCompleted = urlParams.get('psa_completed');
    const nextStep = urlParams.get('next_step');
    
    if (psaCompleted === 'true') {
      // Update user progress to reflect PSA completion
      setUserProgress(prev => ({
        ...prev,
        psa_signed: true,
        onboarding_progress: Math.max(prev.onboarding_progress, 60), // Bump to 60%
        currentStep: nextStep || 'credentialing'
      }));

      // Optional: Show a toast notification
      console.log('🎉 PSA completion detected from redirect!');
  ...
  const onboardingSteps = [
    ...
    },
    {
      ...
      justCompleted: userProgress.psa_signed && window.location.search.includes('psa_completed=true'),
    ...
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        {onboardingSteps.map((step) => (
          <OnboardingCard key={step.id} {...step} justCompleted={step.justCompleted} />
        ))}
      </div>
