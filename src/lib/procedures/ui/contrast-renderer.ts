...
function renderContrastOption(option: ContrastOption, accentColor: string): string {
  ...
  return `
    <button
      type="button"
      class="contrast-option-button card hover-lift transition-all duration-200 border border-gray-100 bg-white/90 hover:bg-white rounded-xl shadow-soft p-4 accent-hover w-full text-left flex items-start gap-4"
    ...
    >
      ...
      <div class="icon-container bg-[#003087]/10 p-3 rounded-lg flex-shrink-0">
        <svg class="w-6 h-6 text-[#003087]" fill="none" stroke="currentColor" viewBox="0 0 24 24">
      ...
      <div class="flex-1 min-w-0">
        ...
        <div class="flex items-center gap-2 mb-1">
          <h4 class="font-semibold text-gray-900 tracking-tight text-base sm:text-lg">
            ${option.label}
          </h4>
          ${option.id === 'without' ? `
        ...
        <!-- Description -->
        <p class="text-gray-500 text-sm leading-snug mb-2">
          ${description}
        </p>
