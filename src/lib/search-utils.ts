...
function expandSynonyms(word: string): string[] {
  ...
  const result = SYNONYM_MAP[normalized] || [normalized];
  
  // Debug
  if (word.toLowerCase().includes('ray')) {
    console.log(`[expandSynonyms] word="${word}" → normalized="${normalized}" → result:`, result);
  }
